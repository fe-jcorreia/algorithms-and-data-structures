...
class Heap():
  ...
  def sift_up(self, index: int):
    ...
    if self.heap_list[index][0] < self.heap_list[(index - 1) // 2][0]:
      aux = self.heap_list[(index - 1) // 2]
      self.heap_list[(index - 1) // 2] = self.heap_list[index]
      self.heap_list[index] = aux
      self.sift_up((index - 1) // 2)
  
  def insert_on_heap(self, value: int, index: int):
    self.heap_list.append((value, index))
    self.length += 1
  ...
  def sift_down(self, index: int):
    ...
    if (2*index) + 1 < self.length and self.heap_list[(2*index) + 1][0] < self.heap_list[min_index][0]:
       min_index = (2*index) + 1
    if (2*index) + 2 < self.length and self.heap_list[(2*index) + 2][0] < self.heap_list[min_index][0]:
       min_index = (2*index) + 2 

    if(self.heap_list[index][0] > self.heap_list[min_index][0]):
  ...
  def remove_from_heap(self, index: int):
    self.heap_list[index][0] = -sys.maxsize - 1
    self.sift_up(index)
    self.extract_min()
